  ...
  app.post("/api/send-notification", async (req, res) => {
    ...
    try {
      if (resend) {
        ...
        if (type === 'invite') {
          subject = `Join ${workspaceName} on FrameTrack`;
          ...
          html = `
            <div style="font-family: sans-serif; padding: 40px; color: #1e293b; background: #f1f5f9;">
              <div style="max-width: 600px; margin: 0 auto; background: white; padding: 40px; border-radius: 24px; box-shadow: 0 10px 15px -3px rgba(0,0,0,0.1);">
                ...
                <p style="font-size: 16px; line-height: 1.6;"><strong>${senderName || 'The Producer'}</strong> has invited you to join their production team <strong>${workspaceName || 'FrameTrack'}</strong> as a <strong>${role || 'Team Member'}</strong>.</p>
          ...
          html = `
            <div style="font-family: sans-serif; padding: 40px; color: #1e293b; background: #f1f5f9;">
              <div style="max-width: 600px; margin: 0 auto; background: white; padding: 40px; border-radius: 24px; box-shadow: 0 10px 15px -3px rgba(0,0,0,0.1);">
                ...
                <div style="margin-top: 32px; padding: 24px; background: #f8fafc; border-radius: 16px; border: 1px solid #e2e8f0;">
                  <p style="margin: 0; font-size: 14px; color: #64748b;">Workspace: ${workspaceName || 'FrameTrack'}</p>
                </div>
              </div>
            </div>
          `;
        ...
        const fromName = process.env.RESEND_FROM_NAME || 'FrameTrack';
